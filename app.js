...
(function () {
    ...
    window.updatePremiumLocks = function () {
        ...
        if (subText) {
            if (active) {
                ...
                subText.textContent = dateStr ? `মেয়াদ: ${dateStr}` : 'সব বিষয় আনলক আছে';
            } else {
    ...
    window.redeemUnlockCode = function () {
        ...
        if (validCodes.includes(entered)) {
            const expireAt = unlockConfig.examEndDate
                ? new Date(unlockConfig.examEndDate + 'T23:59:59').getTime()
            ...
            statusEl.textContent = `✅ প্রিমিয়াম চালু হয়েছে! পরীক্ষা পর্যন্ত (${dateStr}) সব বিষয় পড়তে পারবে।`;
        } else {
            statusEl.style.color = '#ff1744';
